ui.module('framework').add('introduce', {
    i18n: ui.i18n('zh', {
        ...
        '<section>' +
        '<p>jQuery做到了自己的理念“write less do more”，单就链式操作就充分体现了这一非常犀利且具有美感的系统理念。</p>' +
        ...
        '</section>' +
        '<header>个人觉得应该增强这四点：</header>' +
        '<section>' +
        '<ul>' +
        '<li>基于Nodejs开发，使用相关工具</li>' +
        '<li>基于视觉和交互来指导组件的开发，是统一且可复用的组件化</li>' +
        '<li>基于数据驱动视图，引入视图生命周期和数据监听</li>' +
        '<li>支持特定应用场景的技术组合和单点优化</li>' +
        '</ul>' +
        '</section>' +
        '<header>以下是jQueryApp已经实现的特性：</header>',
